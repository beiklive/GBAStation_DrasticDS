...
void Buffer::read(const vk::Vulkan& vk, void* data, size_t length) const {
    if (!data || length > this->size)
        throw ls::vulkan_error("invalid host buffer read");

    void* mapped{};
    auto res = vk.df().MapMemory(vk.dev(), *this->memory, 0, length, 0, &mapped);
    if (res != VK_SUCCESS)
        throw ls::vulkan_error(res, "vkMapMemory() failed while reading buffer");
