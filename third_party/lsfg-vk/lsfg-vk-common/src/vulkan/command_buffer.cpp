...
void CommandBuffer::begin(const vk::Vulkan& vk) const {
    auto res = vk.df().ResetCommandBuffer(*this->commandBuffer, 0);
    if (res != VK_SUCCESS)
        throw ls::vulkan_error(res, "vkResetCommandBuffer() failed");

    const VkCommandBufferBeginInfo beginInfo = {
        .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
        .flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT
    };
    res = vk.df().BeginCommandBuffer(*this->commandBuffer, &beginInfo);
    if (res != VK_SUCCESS)
...
void CommandBuffer::clearColorImage(const vk::Vulkan& vk, const vk::Image& image,
    ...
    vk.df().CmdClearColorImage(*this->commandBuffer,
        image.handle(), VK_IMAGE_LAYOUT_GENERAL, &color, 1, &range);
}
