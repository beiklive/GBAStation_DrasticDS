...
namespace {
    ...
    ls::owned_ptr<VkPipelineCache> createPipelineCache(
        ...
        if (res != VK_SUCCESS && !cache.empty()) {
            // Driver/package changes can invalidate an on-disk cache. Retry
            // empty instead of making frame generation permanently fail.
...
VulkanDeviceFuncs vk::initVulkanDeviceFuncs(const VulkanInstanceFuncs& f, VkDevice d,
    ...
    return {
        ...
        // Optional for the same-device Horizon backend. The Linux layer uses
        // these, but NVK on Switch intentionally exposes no FD transport.
